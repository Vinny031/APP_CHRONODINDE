...
export function useTimer(enclos: Ref<EnclosState[]>, enclosActifId: Ref<number>) {
  ...
  function demarrerTimer(enclosId: number = enclosActifId.value) {
    ...
    if (runtime.state === 'idle') {
      ...
      runtime.tempsRestant = tempsSourcePourEnclos(enc)
      ...
      runtime.tempsSecondaireRestant = jaugeSecondaire ? estimerJaugePourEnclos(enc, jaugeSecondaire).tempsSecondes : null
    }
  ...
  const enclosTimers = computed<EnclosTimerInfo[]>(() =>
    enclos.value.map(enc => {
      ...
      return {
        ...
        tempsSecondaire: enc.jaugesActives.length >= 2 ? runtime.tempsSecondaireRestant : null,
